...
class CapeUsageGraph {
  ...
  async _fetchDataForViewport() {
    ...
    if (capeDataCache.hasRecentlyFailed(fetchStart, fetchEnd)) {
      console.warn(`Skipping fetch for cape ${this.capeId}: recent request failed`);
      return;
    }
    
    // Check if we already have this data in cache
    if (capeDataCache.hasTimeRange(fetchStart, fetchEnd)) {
      const data = capeDataCache.getDataForTimeRange(fetchStart, fetchEnd);
    ...
    try {
      // Fetch data from API
      const apiUrl = `https://capes.api.fyz.sh/${this.capeId}/usage?startTime=${fetchStart}&endTime=${fetchEnd}`;
      ...
      // Add to cache
      capeDataCache.addToCache(fetchStart, fetchEnd, formattedData);
      ...
      capeDataCache.addFailedRequest(fetchStart, fetchEnd);
  ...
  // Modify the validateViewport method to call fetchDataForViewport
  validateViewport() {
    const now = new Date().getTime();
    const minAllowedTime = new Date('2010-01-01').getTime();
    const maxAllowedTime = now + 7 * 24 * 60 * 60 * 1000;
...
// Class for cape data cache
class CapeDataCache {
  constructor() {
    this.cache = new Map(); // Map for storing cached data
    this.pendingRequests = new Map(); // Map for storing pending requests
    this.cacheDuration = 24 * 60 * 60 * 1000; // Cache validity duration: 24h in milliseconds
    this.failedRequests = new Map(); // Track failed requests to avoid retrying too soon
    this.failedRequestBackoff = 5 * 60 * 1000; // Wait 5 minutes before retrying failed requests
  }

  // Check if a time range is already cached and still valid
  hasTimeRange(startTime, endTime) {
    // Iterate through all cached ranges to see if they contain the requested range
    for (const [key, data] of this.cache.entries()) {
      const [cachedStart, cachedEnd, timestamp] = key.split('_').map(t => parseInt(t));
      
      // Check if the requested range is entirely contained within a cached range
      if (cachedStart <= startTime && cachedEnd >= endTime) {
        // Check if the cache is still valid (less than 24h)
        const now = Date.now();
  ...
  // Check if this request recently failed (to avoid spamming)
  hasRecentlyFailed(startTime, endTime) {
    for (const [key, timestamp] of this.failedRequests.entries()) {
      const [failedStart, failedEnd] = key.split('_').map(t => parseInt(t));
      
      // Check if current request overlaps with a failed one
  ...
  addFailedRequest(startTime, endTime) {
  ...
  cleanupFailedRequests() {
    const now = Date.now();
    for (const [key, timestamp] of this.failedRequests.entries()) {
      if (now - timestamp > this.failedRequestBackoff) {
  ...
  getDataForTimeRange(startTime, endTime) {
    ...
    // Iterate through all cached ranges
    for (const [key, data] of this.cache.entries()) {
      const [cachedStart, cachedEnd, timestamp] = key.split('_').map(t => parseInt(t));
      ...
      // If the cached range overlaps with the requested range
      if (!(cachedEnd < startTime || cachedStart > endTime)) {
        // Filter data that is within the requested range
  ...
  // Add data to cache
  addToCache(startTime, endTime, data) {
    const timestamp = Date.now(); // Add current timestamp for TTL
    const key = `${startTime}_${endTime}_${timestamp}`;
    
    // Check if there are ranges that overlap
    const overlappingRanges = [];
    for (const [existingKey, existingData] of this.cache.entries()) {
      const [cachedStart, cachedEnd, cachedTimestamp] = existingKey.split('_').map(t => parseInt(t));
      
      // If the new range overlaps with an existing range
      if (!(cachedEnd < startTime || cachedStart > endTime)) {
        overlappingRanges.push(existingKey);
      }
    }
    ...
    if (overlappingRanges.length > 0) {
      ...
      for (const rangeKey of overlappingRanges) {
        const [cachedStart, cachedEnd] = rangeKey.split('_').map(t => parseInt(t));
        const cachedData = this.cache.get(rangeKey);
        
        // Merge data
        mergedData = mergedData.concat(cachedData);
        
        // Update time limits
        newStart = Math.min(newStart, cachedStart);
        newEnd = Math.max(newEnd, cachedEnd);
        
        // Delete the old range
        this.cache.delete(rangeKey);
      }
      
      // Add the merged range with a new timestamp
      mergedData = this.deduplicateAndSort(mergedData);
      this.cache.set(`${newStart}_${newEnd}_${timestamp}`, mergedData);
    } else {
      // Add the new range simply
  ...
  addPendingRequest(startTime, endTime, promise) {
    const key = `${startTime}_${endTime}`;
    this.pendingRequests.set(key, promise);
  }

  // Check if a request is pending
  hasPendingRequest(startTime, endTime) {
    for (const [key, promise] of this.pendingRequests.entries()) {
      const [cachedStart, cachedEnd] = key.split('_').map(t => parseInt(t));
      
      // Check if the requested range is entirely contained in a pending request
      if (cachedStart <= startTime && cachedEnd >= endTime) {
        return promise;
      }
    }
  ...
  removePendingRequest(startTime, endTime) {
    const key = `${startTime}_${endTime}`;
    this.pendingRequests.delete(key);
  }
}

// Global cache for cape data
const capeDataCache = new CapeDataCache();

// Function for retrieving cape usage data
async function getCapeUsageData(capeId, timeframe = 'week') {
  ...
  // Check if this request recently failed to avoid spamming failed requests
  if (capeDataCache.hasRecentlyFailed(startTimestamp, endTimestamp)) {
    console.warn(`Skipping fetch for cape ${capeId}: recent request failed`);
    
    // Show error message in all graph containers
  ...
  if (capeDataCache.hasTimeRange(startTimestamp, endTimestamp)) {
    return capeDataCache.getDataForTimeRange(startTimestamp, endTimestamp);
  }
  
  // Check if there is a pending request for this range
  const pendingRequest = capeDataCache.hasPendingRequest(startTimestamp, endTimestamp);
  if (pendingRequest) {
  ...
  const apiUrl = `https://capes.api.fyz.sh/${capeId}/usage?startTime=${startTimestamp}&endTime=${endTimestamp}`;
  ...
  try {
    ...
    const requestPromise = fetch(apiUrl)
      ...
      .then(data => {
        ...
        // Add to cache
        capeDataCache.addToCache(startTimestamp, endTimestamp, formattedData);
        capeDataCache.removePendingRequest(startTimestamp, endTimestamp);
        
        return formattedData;
      })
      .catch(error => {
        console.error('Error fetching cape usage data:', error);
        
        // Record this as a failed request to avoid retrying too often
        capeDataCache.addFailedRequest(startTimestamp, endTimestamp);
        ...
        // In case of failure, generate simulated data
        const simulatedData = CapeUsageGraph.generateMockData(
          timeframe === 'day' ? 1 :
          timeframe === 'week' ? 7 :
          timeframe === 'month' ? 30 :
          timeframe === 'year' ? 365 : 30
        ...
        // Do not add simulated data to cache to allow future retries
        capeDataCache.removePendingRequest(startTimestamp, endTimestamp);
    ...
    capeDataCache.addPendingRequest(startTimestamp, endTimestamp, requestPromise);
    
    return requestPromise;
  } catch (error) {
    console.error('Unexpected error in getCapeUsageData:', error);
...
const createUsageGraphCard = (capeHash) => {
...
const initializeGraph = (capeHash) => {
  ...
  const graph = new CapeUsageGraph(canvas, {
    ...
    capeId: capeHash, // Pass the capeId for data fetching
    statElements: {
      current: document.getElementById('stat-current'),
      max: document.getElementById('stat-max'),
      min: document.getElementById('stat-min'),
  ...
  // Load data
  getCapeUsageData(capeHash, 'week').then(data => {
    graph.setData(data);
